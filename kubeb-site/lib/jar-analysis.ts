...
// Generates a "test build" — the same JAR bytes, with only the declared
// Minecraft version compatibility field rewritten to a new target version.
// This does NOT change any compiled code. It exists so a mod can be loaded
// by Fabric Loader on a version it wasn't originally built for, purely so a
// real person can launch it and see what actually happens. It is not a real
// port: nothing about the mod's behavior against the new version's APIs is
// verified or guaranteed. Only Fabric mods are supported currently, since
// that's the loader format this function edits.
export async function generateTestBuild(file: File, targetVersion: string): Promise<Blob> {
  const buffer = await file.arrayBuffer();
  const zip = await JSZip.loadAsync(buffer);

  const fabricEntry = zip.file("fabric.mod.json");
  if (!fabricEntry) {
    throw new Error("Test builds are only supported for Fabric mods (no fabric.mod.json found).");
  }

  const raw = await fabricEntry.async("string");
  const data = JSON.parse(raw);
  if (!data.depends) data.depends = {};
  data.depends.minecraft = targetVersion;

  zip.file("fabric.mod.json", JSON.stringify(data, null, "\t"));
