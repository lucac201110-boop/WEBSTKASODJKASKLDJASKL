...
import { analyzeJar, generateTestBuild, JarAnalysis } from "@/lib/jar-analysis";
import { converterVersions } from "@/lib/data";
import CompatScan from "@/components/converter/CompatScan";
...
export default function ConverterPage() {
  ...
  const [testVersion, setTestVersion] = useState(converterVersions[converterVersions.length - 1]);
  ...
  async function downloadTestBuild() {
    if (!uploadedFile) return;
    setBuildingTest(true);
    try {
      const blob = await generateTestBuild(uploadedFile, testVersion);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${(analysis?.modId ?? "mod")}-${testVersion}-test.jar`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : "Could not generate test build.");
      setStage("error");
    } finally {
                ...
                {analysis.loader === "Fabric" && (
                  <div className="rounded-2xl border border-line bg-card p-6">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <span className="text-[13.5px] font-semibold">Generate Test Build</span>
                    </div>
                    <p className="mb-4 text-[12px] leading-relaxed text-ink-secondary">
                      Repackages this JAR with only its declared Minecraft version changed, so Fabric
                      Loader will let it load on a version it wasn&apos;t originally built for. The
                      compiled code isn&apos;t modified — this doesn&apos;t verify or guarantee the mod
                      actually works there, it just lets you launch it and find out.
                    </p>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                      ...
                      <button
                        onClick={downloadTestBuild}
                        disabled={buildingTest}
                        className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-accent/40 bg-accent/[0.08] px-4 py-2.5 text-[13px] font-semibold text-ink transition-colors hover:bg-accent/[0.14] disabled:opacity-50"
                      >
                        {buildingTest ? (
                        ...
                        Download test build for {testVersion}
                      </button>
                    </div>
                  </div>
                )}

                {analysis.loader === "Fabric" && uploadedFile && (
                  <CompatScan file={uploadedFile} detectedVersion={analysis.minecraftRange.replace(/[^\d.]/g, "")} />
                )}
